...
async function run() {
  ...
  for (const table of tables) {
    ...
    const conditions = uuidCols.map(col => `"${col}"::text = '11b96988-e4fb-4b5d-ad20-923dae2f24e2'`).join(' OR ');
  ...
  if (!foundSomething) {
    console.log("No linked records found for the lot ID");
  }
